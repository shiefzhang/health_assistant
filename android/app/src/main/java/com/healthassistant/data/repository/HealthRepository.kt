...
class HealthRepository(
    ...
    suspend fun importAllGlucose(records: List<GlucoseRecord>): Int {
        return mergeImport(records, glucoseDao.getAllIncludingDeleted()) { it.id }
    }
    suspend fun importAllWeight(records: List<WeightRecord>): Int {
        return mergeImport(records, weightDao.getAllIncludingDeleted()) { it.id }
    }
    suspend fun importAllBp(records: List<BloodPressureRecord>): Int {
        return mergeImport(records, bpDao.getAllIncludingDeleted()) { it.id }
    }
